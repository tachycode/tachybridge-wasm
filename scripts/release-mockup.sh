...
PACKAGE_NAME="tachybridge-mockup-rosbridge"
PACKAGE_DIR="mockup-rosbridge"
GIT_TAG="${PACKAGE_NAME}-v${VERSION}"

if [[ -z "$VERSION" ]]; then
  echo "Usage: npm run release:mockup -- <version> [npm-dist-tag]" >&2
  echo "Example: npm run release:mockup -- 0.1.1 latest" >&2
  exit 1
fi
